...
import { ArrowUpRight, Clock3, Eye, Heart, MessageCircle, Radar, Repeat2, Sparkles } from "lucide-react";
...
function scoreStyle(score: number) { return score >= 85 ? "bg-success/15 text-success" : score >= 70 ? "bg-accent-tint text-accent-ink" : score >= 55 ? "bg-warning/15 text-warning" : "bg-panel-raised text-neutral-600"; }
...
export function RadarExperience() {
  ...
  useEffect(() => { const controller = new AbortController(); fetch("/api/x-radar", { signal: controller.signal }).then((response) => { if (!response.ok) throw new Error("Could not load the radar"); return response.json(); }).then(setData).catch((reason) => { if (reason.name !== "AbortError") setError("The radar is temporarily unavailable. Please try again later."); }); return () => controller.abort(); }, []);
  ...
  return <div className="min-h-0 flex-1 overflow-y-auto bg-bg px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
    <div className="mx-auto max-w-[1120px]">
      ...
      {!data && !error ? <div aria-live="polite" className="flex min-h-56 items-center justify-center rounded-xl border border-line bg-surface text-sm text-neutral-600"><Sparkles className="mr-2 animate-pulse text-accent" size={18} aria-hidden />Reading the latest signals…</div> : posts.length === 0 ? <div className="rounded-xl border border-line bg-surface px-6 py-16 text-center"><p className="font-semibold text-ink">No posts available yet.</p><p className="mt-2 text-sm text-neutral-600">The radar will check again at the next scheduled scan.</p></div> : <div className="overflow-hidden rounded-xl border border-line bg-surface">
        <div className="hidden grid-cols-[82px_150px_minmax(220px,1.3fr)_minmax(220px,1fr)_150px_52px_56px] gap-3 border-b border-line bg-panel px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wide text-neutral-500 xl:grid"><span>Score</span><span>Author</span><span>Post</span><span>Why reply</span><span>Engagement</span><span>Age</span><span>Actions</span></div>
        <ul className="divide-y divide-line">{posts.map((post) => <li key={post.id} className="grid gap-4 p-4 transition-colors hover:bg-panel/50 xl:grid-cols-[82px_150px_minmax(220px,1.3fr)_minmax(220px,1fr)_150px_52px_56px] xl:gap-3">
          <div><span className={`inline-flex min-w-14 flex-col items-center rounded-lg px-2 py-1.5 ${scoreStyle(post.opportunityScore)}`}><strong className="text-xl leading-none">{post.opportunityScore}</strong><span className="mt-1 text-[10px] font-semibold">{post.label}</span></span></div>
