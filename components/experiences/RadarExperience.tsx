  ...
  return <div className="min-h-0 flex-1 overflow-y-auto bg-bg px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
    <div className="mx-auto max-w-[1120px]">
      ...
      {!data && !error ? <div aria-live="polite" className="flex min-h-56 items-center justify-center rounded-xl border border-line bg-surface text-sm text-neutral-600"><Sparkles className="mr-2 animate-pulse text-accent" size={18} aria-hidden />Reading the latest signals…</div> : posts.length === 0 ? <div className="rounded-xl border border-line bg-surface px-6 py-16 text-center"><p className="font-semibold text-ink">No conversation cleared the quality gates.</p><p className="mt-2 text-sm text-neutral-600">The filter would rather return nothing than fill the page with weak matches.</p></div> : <div className="rounded-xl border border-line bg-surface">
        <div className="hidden grid-cols-[104px_180px_minmax(360px,1fr)_190px_112px] gap-5 rounded-t-xl border-b border-line bg-panel px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wide text-neutral-500 xl:grid"><span>Fit</span><span>Author</span><span>Post</span><span>Public activity</span><span aria-label="Actions" /></div>
        <ul className="divide-y divide-line">{posts.map((post) => <li key={post.id} className="grid items-start gap-4 p-4 transition-colors hover:bg-panel/50 xl:grid-cols-[104px_180px_minmax(360px,1fr)_190px_112px] xl:gap-5">
          ...
          <div><Metrics post={post} /></div>
          <div className="flex min-h-8 items-center justify-end gap-2"><button type="button" onClick={() => void copySuggestedReply(post)} aria-label={`Copy a suggested reply prompt for ${post.author.name}'s post`} title={copiedPostId === post.id ? "Suggested reply prompt copied" : "Copy suggested reply prompt"} className="inline-flex size-8 cursor-pointer items-center justify-center rounded-md text-ink transition hover:scale-105 hover:bg-panel-raised hover:text-accent active:scale-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent">{copiedPostId === post.id ? <Check size={15} aria-hidden /> : <Copy size={15} aria-hidden />}</button><a href={post.url} target="_blank" rel="noopener noreferrer" aria-label={`Open ${post.author.name}'s post on X in a new tab`} title="Open post on X" className="inline-flex size-8 cursor-pointer items-center justify-center rounded-md text-ink transition hover:scale-105 hover:bg-panel-raised hover:text-accent active:scale-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"><XBrandMark /></a></div>
        </li>)}</ul>
      </div>}
