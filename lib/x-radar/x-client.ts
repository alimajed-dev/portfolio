...
type ApiPost = { id?: string; text?: string; author_id?: string; created_at?: string; public_metrics?: Record<string, number>; referenced_tweets?: Array<{type: string}> };
type ApiUser = { id?: string; name?: string; username?: string; description?: string; verified?: boolean; profile_image_url?: string; public_metrics?: Record<string, number> };
...
export async function searchRecentPosts(signal?: AbortSignal): Promise<XPost[]> {
  ...
  const topics = '(AI OR developer OR software OR product OR build OR coding OR packages OR dependencies OR "generated code" OR RAG OR LLM OR model OR prompt OR Claude OR OpenAI OR ChatGPT OR YouTube OR API)';
  const substance = '("more important" OR harder OR hardest OR why OR strategy OR annoying OR problem OR outage OR down OR incident OR quality OR security OR reliability OR tradeoff OR debate OR controversial OR announces OR launches OR releases)';
  ...
  return (payload.data ?? []).flatMap((post): XPost[] => {
    ...
    return [{ id: post.id, text: post.text, createdAt: post.created_at, author: { id: user.id, name: user.name, username: user.username, description: user.description, profileImageUrl: user.profile_image_url, followers: user.public_metrics?.followers_count, verified: user.verified }, metrics: { likes: m.like_count ?? 0, replies: m.reply_count ?? 0, reposts: m.retweet_count ?? 0, quotes: m.quote_count ?? 0, impressions: m.impression_count } }];
  });
}
