...
export async function searchRecentPosts(signal?: AbortSignal): Promise<XPost[]> {
  ...
  return (payload.data ?? []).flatMap((post): XPost[] => {
    ...
    return [{ id: post.id, text: post.text, createdAt: post.created_at, format: post.article ? "article" : post.note_tweet ? "note" : "standard", author: { id: user.id, name: user.name, username: user.username, description: user.description, profileImageUrl: user.profile_image_url, followers: user.public_metrics?.followers_count, verified: user.verified }, metrics: { likes: m.like_count ?? 0, replies: m.reply_count ?? 0, reposts: m.retweet_count ?? 0, quotes: m.quote_count ?? 0, impressions: m.impression_count } }];
  });
}
