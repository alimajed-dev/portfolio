  ...
  return <article className="min-h-0 flex-1 overflow-y-auto px-4 py-6 sm:px-6 lg:py-12">
    <div className="mx-auto max-w-[680px]">
      ...
      <div className="mt-8 space-y-7 text-[13px]/[1.65] text-neutral-600 sm:text-[14px]/[1.65]">
        <section><h2 className="text-base font-semibold text-ink">Information processed</h2><p className="mt-2">Up to 10 recent public candidate posts per scheduled scan, plus quoted-source context returned in the same response. This can include post text and identifiers, timestamps, public author names, handles and profile descriptions, follower and posting-activity signals, verification status, and public engagement metrics returned by the X API. The radar does not access protected posts, Direct Messages, email addresses, private metrics, or precise location data.</p></section>
        <section><h2 className="text-base font-semibold text-ink">How it is used</h2><p className="mt-2">The service filters unsuitable results and ranks conversations using existing interactions, interaction velocity, professional relevance, author authority, view reach, and room to contribute. It never posts, replies, likes, reposts, follows, or messages anyone. Every decision to engage is made manually on X.</p></section>
        ...
        <section><h2 className="text-base font-semibold text-ink">Storage and removal</h2><p className="mt-2">Only the latest successful result set and usage counters are cached privately on the Railway volume. The public snapshot expires after the configured retention period, and new successful scans replace it. Content that is deleted, protected, suspended, withheld, or otherwise unavailable will be removed or updated as soon as reasonably possible. X or the applicable account owner may request removal at any time; requests are handled within 24 hours.</p></section>
